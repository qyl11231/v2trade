...
import com.qyl.v2trade.business.mapper.StrategyDefinitionMapper;
import com.qyl.v2trade.business.model.dto.StrategyCreateRequest;
import com.qyl.v2trade.business.model.entity.StrategyDefinition;
import com.qyl.v2trade.business.service.StrategyDefinitionService;
import com.qyl.v2trade.business.service.StrategyParamService;
...
public class StrategyDefinitionServiceImpl extends ServiceImpl<StrategyDefinitionMapper, StrategyDefinition> implements StrategyDefinitionService {
    ...
    @Autowired
    @Lazy
    private com.qyl.v2trade.business.service.TradingPairService tradingPairService;

    @Autowired
    @Lazy
    private com.qyl.v2trade.business.service.SignalConfigService signalConfigService;
    ...
    public StrategyDefinition updateStrategyComplete(Long strategyId, Long userId, StrategyCreateRequest request) {
        ...
        try {
            ...
            // 3. 更新交易对配置
            // 获取现有的交易对配置
            List<com.qyl.v2trade.business.model.entity.StrategySymbol> existingSymbols = strategySymbolService.listByStrategyId(strategyId);
            java.util.Set<Long> existingSymbolIds = existingSymbols.stream()
                    .map(com.qyl.v2trade.business.model.entity.StrategySymbol::getId)
            ...
            // 4. 更新信号订阅配置
            // 获取现有的信号订阅配置
            List<com.qyl.v2trade.business.model.entity.StrategySignalSubscription> existingSubs = strategySignalSubscriptionService.listByStrategyId(strategyId);
            java.util.Set<Long> existingSubIds = existingSubs.stream()
                    .map(com.qyl.v2trade.business.model.entity.StrategySignalSubscription::getId)
                    .collect(java.util.stream.Collectors.toSet());
    ...
    @Override
    public com.qyl.v2trade.business.model.dto.StrategyDetailVO getStrategyDetail(Long strategyId, Long userId) {
        ...
        // 构建详情VO
        com.qyl.v2trade.business.model.dto.StrategyDetailVO detailVO = new com.qyl.v2trade.business.model.dto.StrategyDetailVO();

        // 1. 策略定义
        com.qyl.v2trade.business.model.dto.StrategyDefinitionVO definitionVO = new com.qyl.v2trade.business.model.dto.StrategyDefinitionVO();
        org.springframework.beans.BeanUtils.copyProperties(strategy, definitionVO);
        detailVO.setDefinition(definitionVO);

        // 2. 策略参数
        com.qyl.v2trade.business.model.entity.StrategyParam param = strategyParamService.getByStrategyId(strategyId);
        if (param != null) {
            com.qyl.v2trade.business.model.dto.StrategyParamVO paramVO = new com.qyl.v2trade.business.model.dto.StrategyParamVO();
            org.springframework.beans.BeanUtils.copyProperties(param, paramVO);
            detailVO.setParam(paramVO);
        }

        // 3. 交易对列表
        List<com.qyl.v2trade.business.model.entity.StrategySymbol> symbols = strategySymbolService.listByStrategyId(strategyId);
        if (symbols != null && !symbols.isEmpty()) {
            // 获取所有交易对ID，批量查询交易对信息
            List<Long> tradingPairIds = symbols.stream()
                    .map(com.qyl.v2trade.business.model.entity.StrategySymbol::getTradingPairId)
                    .distinct()
                    .collect(java.util.stream.Collectors.toList());

            java.util.Map<Long, com.qyl.v2trade.business.model.entity.TradingPair> tradingPairMap = 
                    tradingPairService.listByIds(tradingPairIds).stream()
                            .collect(java.util.stream.Collectors.toMap(
                                    com.qyl.v2trade.business.model.entity.TradingPair::getId, 
                                    pair -> pair
                            ));

            List<com.qyl.v2trade.business.model.dto.StrategySymbolVO> symbolVOList = symbols.stream()
                    .map(symbol -> {
                        com.qyl.v2trade.business.model.dto.StrategySymbolVO vo = new com.qyl.v2trade.business.model.dto.StrategySymbolVO();
                        org.springframework.beans.BeanUtils.copyProperties(symbol, vo);
                        com.qyl.v2trade.business.model.entity.TradingPair pair = tradingPairMap.get(symbol.getTradingPairId());
                        if (pair != null) {
        ...
        // 4. 信号订阅列表
        List<com.qyl.v2trade.business.model.entity.StrategySignalSubscription> subscriptions = 
        ...
        if (subscriptions != null && !subscriptions.isEmpty()) {
            ...
            List<Long> signalConfigIds = subscriptions.stream()
                    .map(com.qyl.v2trade.business.model.entity.StrategySignalSubscription::getSignalConfigId)
                    .distinct()
                    .collect(java.util.stream.Collectors.toList());

            java.util.Map<Long, com.qyl.v2trade.business.model.entity.SignalConfig> signalConfigMap = 
                    signalConfigService.listByIds(signalConfigIds).stream()
                            .collect(java.util.stream.Collectors.toMap(
                                    com.qyl.v2trade.business.model.entity.SignalConfig::getId, 
                                    config -> config
                            ));

            List<com.qyl.v2trade.business.model.dto.StrategySignalSubscriptionVO> subscriptionVOList = subscriptions.stream()
                    .map(subscription -> {
                        com.qyl.v2trade.business.model.dto.StrategySignalSubscriptionVO vo = 
                                new com.qyl.v2trade.business.model.dto.StrategySignalSubscriptionVO();
                        org.springframework.beans.BeanUtils.copyProperties(subscription, vo);
                        com.qyl.v2trade.business.model.entity.SignalConfig config = signalConfigMap.get(subscription.getSignalConfigId());
                        if (config != null) {
